...
class ChatRoomController extends ControllerBase {
  ...
  /**
   * Sohbet geçmişini temizle - ÇOK BASİT TEST
   */
  public function clearChatHistory($room_id, Request $request) {
    ...
    if ($request->query->get('confirm') === '1') {
      ...
      return $this->redirect('chat_space.room', ['room_id' => $room_id]);
    }

    // Onay sayfasını göster
    $confirm_url = Url::fromRoute('chat_space.clear_history', ['room_id' => $room_id], ['query' => ['confirm' => '1']])->toString();
    $cancel_url = Url::fromRoute('chat_space.room', ['room_id' => $room_id])->toString();
